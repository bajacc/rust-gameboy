...
impl Renderer {
    pub fn new(has_tile: bool, has_bg: bool) -> Self {
        ...
        if has_tile {
            let window = Window::new("GameBoy", TILE_WINDOW_WIDTH, TILE_WINDOW_HEIGHT, WindowOptions::default()).unwrap();
            tiles_window = Some(window);
        }

        let mut bg_window = Vec::new();
        if has_bg {
            let window1 = Window::new("GameBoy", BG_WINDOW_WIDTH, BG_WINDOW_HEIGHT, WindowOptions::default()).unwrap();
            bg_window.push(window1);
            let window2 = Window::new("GameBoy", BG_WINDOW_WIDTH, BG_WINDOW_HEIGHT, WindowOptions::default()).unwrap();
